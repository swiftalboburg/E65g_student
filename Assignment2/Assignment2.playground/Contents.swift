...
//let mycell = Cell(position: (row: 2, col: 2), state: CellState.alive)
...
/*
 Indicate that the name of the parameters are not required when calling the function
 */
/*:
...
/*
 Is a function that takes two parameters and returns a generic T, which in this case is a cell
 */
/*:
...
/*
 T is a type, which in this context will be Cell
 */
...
/*
 represent the 8 neighbor cells
 */
...
/*
 "Of" is a custom argument label, I would use it when calling the function to find the neighbors "of" the cell being analized.
 */
/*:
...
extension Grid {
    ...
    func neighbors(of cell: Cell) -> [Position] {     // uses custom argument label "of" ?
        return Grid.offsets.map  {
            // ** Your Problem 9 Code goes here! replace the following line **
            //return Position(row: $0, col: $1)
            return Position( (cell.position.row + $0.row + rows) % rows, (cell.position.col + $0.col + cols) % cols  )
...
/*
 Because all the parameter types are specified.  It is not a generic function, whose placeholder types are determined when called
 */
...
//   // ** Your Problem 13 code goes here! **
    return Int(arc4random_uniform(3)) == 2 ? .alive : .empty
    }
grid.numLiving
...
/*
 The "next" generation, the next iteration of the game
 */
/*:
 ## Problem 22:
